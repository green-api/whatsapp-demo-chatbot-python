...
apiTokenInstance
```
После получения данных параметров, откройте файл `bot.py` и введите `idInstance` и `apiTokenInstance` в следующие обьекты на 11-й строке. 
...
Ссылки должны вести на файлы из облачного хранилища или открытого доступа. На 75-й строке содержится следующий код:
```python
elif message == "2":
...
Данная строка после изменения будет в сдедующем формате:
```python
elif message == "2":
    notification.api.sending.sendFileByUrl(
        chatId=notification.chat,
...
```
Таким же образом введите ссылку и название для картинки на 83-й строке:
```python
elif message == "3":
    notification.api.sending.sendFileByUrl(
        chatId=notification.chat,
...
[2] - Русский
```
Ответьте 1 или 2, чтобы выбрать язык для дальнейшего общения. После того, как вы отправите 2, чатбот пришлет приветсвенное сообщение на русском языке:
```
...
```
Выбрав число из списка и отправив его, чатбот ответит каким API был отправлен данный тип сообщения и прикрепит ссылку на информацию о API. 
...
```
Извините, я не совсем вас понял, напишите [меню], что посмотреть возможные опции
```
...
```
Инициализация чатбота происходит на 14-й строке:
```python
bot = GreenAPIBot(id_instance, api_token_instance)
```
Каждое сообщение отправленное чатботу обрабатывается на 20-й строке:
```python
