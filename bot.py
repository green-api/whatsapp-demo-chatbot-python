...
def main_menu_option_1_handler(notification: Notification) -> None:
    ...
    notification.answer(first_option_answer_text, link_preview=config.link_preview, typing_time=1000)
...
def main_menu_option_4_handler(notification: Notification) -> None:
    ...
    notification.answer(fourth_option_answer_text, link_preview=config.link_preview, typing_time=1000)
    notification.api.sending.sendFileByUrl(
        notification.chat,
        url_file,
        file_name,
...
def main_menu_option_6_handler(notification: Notification) -> None:
    ...
    notification.answer(sixth_option_answer_text, link_preview=config.link_preview, typing_time=1000)
...
def main_menu_option_7_handler(notification: Notification) -> None:
    ...
    notification.answer(seventh_option_answer_text, link_preview=config.link_preview, typing_time=1000)
...
def main_menu_option_8_handler(notification: Notification) -> None:
    ...
    notification.answer(eighth_option_answer_text, link_preview=config.link_preview, typing_time=1000)
...
def main_menu_option_9_handler(notification: Notification) -> None:
    ...
    notification.answer(ninth_option_answer_text, link_preview=config.link_preview, typing_time=1000)
...
@debug_profiler(logger=logger)
def interactive_buttons_handler(notification: Notification) -> None:
    """
    ...
    try:
        if "interactiveButtonsResponse" in event:
            button_data = event["interactiveButtonsResponse"]
        elif "messageData" in event and "interactiveButtonsResponse" in event["messageData"]:
            button_data = event["messageData"]["interactiveButtonsResponse"]
        else:
            logger.error(f"Не найдены данные кнопки в событии: {event}")
            return

        selected_id = button_data.get("selectedId")
        selected_text = button_data.get("selectedDisplayText")
        ...
        if selected_id == "1":
            ...
            logger.info(f"Нажата кнопка: {selected_text} (ID: {selected_id})")

    except Exception as e:
        logger.exception(f"Error in interactive_buttons_handler: {e}")
        notification.answer("Ошибка обработки кнопки")
        ...
        for vote_data in votes:
            ...
            if voters:
                if option_name == f'{answers_data["poll_option_1"][sender_lang_code]}':
                    notification.answer(
                        f'{answers_data["poll_answer_1"][sender_lang_code]}', typing_time=1000
                    )
                    ...
                    notification.answer(
                        f'{answers_data["poll_answer_2"][sender_lang_code]}', typing_time=1000
                    )
                    ...
                    notification.answer(
                        f'{answers_data["poll_answer_3"][sender_lang_code]}', typing_time=1000
                    )
